...
void adapt_window_to_frame(int *window_beg, int *window_end, int center, int size, int r);
...
double unreach_dst2 = 255*255 + 255*255 + 255*255 + x_size*x_size + y_size*y_size; // max spacial distance squared is x_size-1 + y_size-1
...
for (i = 0; i < y_size; i++)
	for (j = 0; j < x_size; j++){
        ...
        data[i][j][0] = Rs_of_labels[ -parents[i][j] ] + 0.5;
        data[i][j][1] = Gs_of_labels[ -parents[i][j] ] + 0.5;
        data[i][j][2] = Bs_of_labels[ -parents[i][j] ] + 0.5;
    }
